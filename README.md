...
진행 경로는 하나입니다: **시작 안내 → Notebook의 01–07 → 정리 확인**. Notebook과 CLI를 둘 다 실행하지 않습니다.
...
| 03 품질 게이트 | 성능 미달 모델의 **의도된 Failed**와 Registry 등록 차단 |
...
| 07 비용 정리 | endpoint 삭제, Instance Stopped, Cluster 실제 0노드 |
...
Workspace는 이 실습 자산을 관리하는 작업 공간입니다. MLflow는 매개변수·지표·모델 이력을 기록합니다. RMSE는 **작을수록 좋은 예측 오차**이며, Registry는 승인 모델을 버전별로 보관하는 곳입니다.
...
**VM을 멈춰도 비용은 완전히 0이 되지 않습니다.** Premium ACR의 East US 2 공개 표시 가격은 약 **USD 1.6666/일**이며 Private Endpoint·Storage·디스크 등은 별도입니다. 실습을 보관하지 않을 때만 [전용 RG 정리](docs/troubleshooting.md#전체-환경이-더-이상-필요하지-않은-경우)를 수행합니다.
