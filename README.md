...
Terminal 연결이나 설치에 실패해도 **[Studio에서 본인 리소스 정리](docs/troubleshooting.md#terminal-없이-정리하기)**는 가능합니다.

```mermaid
flowchart LR
    A["01 자산 등록"] --> B["02 기본 모델 학습·평가"]
    B --> C["03 별도 모델의 등록 차단"]
    C -->|"등록 차단 확인 후"| D["04 blue 배포"]
    B -.->|"02 통과 모델"| D
    D --> E["05 재학습"]
...
실선은 진행 순서, 점선은 배포할 모델의 출처입니다. **03은 별도 성능 미달 실험이며, 04에는 02의 통과 모델을 배포합니다.** 02와 05에도 각각 품질 게이트가 포함됩니다.
...
| 03 품질 게이트 | 별도 성능 미달 Job의 **의도된 Failed**와 Models 등록 차단 |
