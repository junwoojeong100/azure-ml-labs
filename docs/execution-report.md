...
이 보고서는 최초 커밋 `fcc0d95`에 해당하는 **당시 Azure 실행 기록**입니다. 이후의 학습자 안내·Notebook 개선은 [문서 명확성 평가](guide-quality.md)에 별도로 기록합니다. 해당 문서 개선을 위해 Azure 리소스를 재실행한 것으로 해석하지 않습니다.

**이 페이지는 따라 실행할 가이드가 아닙니다.** 아래 계정·리소스명·게시 경로를 본인의 설정으로 복사하지 않습니다. 새 실습은 [학습자 시작 안내](learner-start.md), 현재 비용·종료 상태 확인은 [정리 안내](troubleshooting.md#리소스-정리)를 따릅니다.
...
**이 보고서의 모델 v1/v2는 당시 등록한 버전**입니다. 현재 가이드는 실행 충돌을 피하려고 모델 버전에 `<LAB_ID>1` / `<LAB_ID>2`를 사용합니다. Data 버전 1/2와 구분하고, 새 실습에서는 Notebook 00 또는 CLI 준비 블록에 출력된 모델 버전을 찾습니다.
...
기준은 **RMSE ≤ 3.0**입니다. 성능 미달 모델은 단순 인프라 실패가 아니라, 평가 보고서의 `approved=false`와 실제 RMSE 초과를 확인했습니다. 등록 명령이 차단되고 거절한 모델 버전 `9`가 Workspace의 Models에 없는 것도 확인했습니다.

기본 모델은 `--force-rerun`으로 실행해 최종 managed network에서 모든 단계가 실제 Compute에서 수행되도록 했습니다. v1/v2의 검증 데이터는 동일한 200행이며 SHA-256이 일치합니다. 데이터에서는 학습 행만 800 → 1,200행으로 늘었고, 재학습 시 모델 설정도 `alpha=1 → 0.1`로 변경했습니다. RMSE 차이를 데이터 증가만의 효과로 해석하지 않습니다.
...
**2026-09-16 최초 게시본**은 첫 화면을 68줄의 Notebook 중심 안내로 줄이고, 상세 CLI 절차와 강사용 네트워크 준비를 분리했습니다. 잘못된 문서 anchor와 로그 확인 위치를 수정했으며, 롤백 뒤 불필요한 재전환을 제거했습니다. 아래 파일 수와 게시 경로도 당시 기록이며, 현재 가이드의 파일 수·배포 상태를 뜻하지 않습니다.
