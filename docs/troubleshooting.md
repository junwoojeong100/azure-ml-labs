...
아래 `<JOB_NAME>`은 `artifacts/runs/<실행명>.json`의 **`job_name` 값**으로 바꿉니다. `baseline-...` 같은 실습 실행명과 Azure Job 이름은 다를 수 있습니다. 나머지는 `config.json`에서 읽은 값을 사용합니다.
...
**순서:** 본인 endpoint 삭제 → Instance 중지 → 전용 Cluster 0노드 확인 → 실행 기록 회수 → 임시 runner 삭제. 공유 Cluster는 [개인 종료 기준](#공유-학습-클러스터의-정리)을 따릅니다.

**학습자 — Instance Terminal에서 실행:**
